      ...
      .then(async (snapshot) => {
        if (snapshot.exists()) {
          ...
          Object.values(snap).map(async (jobOrder) => {
            ...
            await get(child(databaseRef, `Jobs/${jobID}`)).then(async (job) => {
              const snapJob = job.val();
              const jobClientID = snapJob.ClientID;
              const jobServiceProviderID = snapJob.ServiceProviderID;
              await get(child(databaseRef, `Users/${jobClientID}/`)).then(
                async (client) => {
                  const snapClient = client.val();

                  await get(
                    child(databaseRef, `Users/${jobServiceProviderID}/`)
                  ).then(async (serviceProvider) => {
                    const snapServiceProvider = serviceProvider.val();
                    ...
                    const mergedDataString = JSON.stringify(JobOrders, null, 2);
                    // console.log(mergedDataString);
                    setJobOrder(JSON.parse(mergedDataString));
                  });
                }
              );
            });
          ...
          {JobOrder &&
            JobOrder.map((obj, index) => {
              ...
              if (isServiceProvider) {
                ...
                iWhatIsLooking = data.ServiceProvider.ServiceOffered;
              }
              if (
                !status
                  .toLowerCase()
                  .includes(data.JobOrder.Status.toLowerCase())
              ) {
                return;
              }
              return (
                <View style={style.taskItem}>
                  ...
                  <ClientFeed
                    ...
                    image={{
                      uri: iProfile ? iProfile : DefaultProfile,
                    }}
                    hasFavorite={false}
                    onpress={() => {}}
                  />
                  {status.toLowerCase().includes("active") ||
                  status.toLowerCase().includes("proposed") ? (
                    <View style={style.btnActionWrap}>
                      ...
                      <SecondaryButton
                        ...
                        onPress={() => {
                          ...
                          update(ref(database, `Jobs/${data.Job.ID}/`), {
                            Active: true,
                          });
