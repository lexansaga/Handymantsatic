...
export default function ClientServiceFeeds({ navigation, route }) {
  ...
  var SearchKey = route.params.SearchKey;
  ...
  const onRefresh = React.useCallback(() => {
    ...
    getWorkers().then((data) => {
      ...
      Object.values(data).map((worker) => {
        ...
        console.log(
          IsTextEmptyFallback(worker.ServiceOffered, `General Worker`)
        );
        console.log(
          `Search ${SearchKey.toLowerCase()} : ${IsTextEmptyFallback(
        ...
        if (!IsNullOrEmpty(SearchKey)) {
          ...
          feedsArr.push(
            <Feed
              ...
              name={worker.Name}
              price={PriceFormat(IsNullOrEmpty(worker.Rate) ? 0 : worker.Rate)}
              cover={
                IsNullOrEmpty(worker.JobDescription)
                  ? `I ${worker.Name} is always happy to work with you!`
                  : worker.JobDescription
